...
class Travis extends EventEmitter {
    ...
    async travisFetch(url, headers = {}) {
        let transaction

        for (let i = 0; i < this.retryCount; i++) {
            transaction = await this.scheduler.fetch(url, headers, { follow: true })

            const { responseCode, responseBody, info } = transaction

            if (responseCode === 429 || info.error) {
                if (i == this.retryCount) {
                    this.emit('warn', `failed ${url} -> ${responseCode} => ${info.error}`)
                    ...
                    this.emit('debug', `retrying ${url} -> ${i}`)
